...
class LogDetector:
    ...
    IPTABLES_RE = re.compile(r'^[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+\S+\s+kernel:\s+IPTABLES-(DROP|ACCEPT):\s+IN=\S*\s+OUT=\S*\s+.*SRC=(?:\d{1,3}\.){3}\d{1,3}\s+DST=(?:\d{1,3}\.){3}\d{1,3}.*PROTO=(TCP|UDP|ICMP).*$')
    UFW_RE = re.compile(r'^[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+\S+\s+ufw\[\d+\]:\s+\[UFW (ALLOW|BLOCK)\]\s+IN=\S*\s+OUT=\S*\s+SRC=(?:\d{1,3}\.){3}\d{1,3}\s+DST=(?:\d{1,3}\.){3}\d{1,3}.*$')
    ...
    MYSQL_ERROR_RE = re.compile(r'(\S+Z)\s+(\d+)\s+\[ERROR\]\s+\[MY-(\d+)\]\s+\[Server\]\s+(.*)')
    MYSQL_QUERY_RE = re.compile(r'(\S+Z)\s+(\d+)\s+Query\s+(.*);')
    MYSQL_SLOW_RE = re.compile(r'^# Time:')
    POSTGRES_ERROR_RE = re.compile(r'^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}(?:\.\d+)?\s+\w+)\s+\[(\d+)\]\s+(?:\S+@\S+\s+)?(ERROR|FATAL):\s+([0-9A-Z]{5}):\s+(.*)$')
    POSTGRES_AUTH_RE = re.compile(r'(\S+)\s+(\S+)\s+\[(\d+)\].*user=(\w+)\s+database=(\w+)')
    POSTGRES_STATEMENT_RE = re.compile(r'(\S+)\s+(\S+)\s+\[(\d+)\]\s+STATEMENT:\s+(.*);')
    ...
    PHP_FPM_RE = re.compile(r'\[\d{2}-[A-Za-z]{3}-\d{4}\s+\d{2}:\d{2}:\d{2}\]\s+(?:NOTICE|WARNING|ERROR):')
    HAPROXY_RE = re.compile(r'^[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+haproxy\[\d+\]:\s+(?:GET|POST|PUT|DELETE)\s+\S+\s+\d{3}$')
